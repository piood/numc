 ...
 */
int set_matrix(matrix **mat, int rows, int cols, double val) {
    // Task 1.2 TODO
...
int allocate_matrix(matrix **mat, int rows, int cols) {
    if (set_matrix(mat, rows, cols, 0) == 0) {
        return 0;
    }
    // failed
    return 1;
            ...
            for (int j = 0; j < col1 / 4 * 4; j += 4) {
                for (int k = 0; k < col2 / 4 * 4; k += 4) {
                    ...
                    _mm256_storeu_pd(result->data + i,
                                     _mm256_fmadd_pd(tmp_1, tmp_2, tmp_c));
                }
            }
        }
    }
    ...
    for (int i = row1 / 4 * 4; i < row1; i++) {
        for (int j = col1 / 4 * 4; j < col1; j++) {
            for (int k = col2 / 4 * 4; k < col2; k++) {
                result->data[i * col2 + k] +=
                    mat1->data[i * col1 + j] * mat2->data[j * col2 + k];
            }
        }
    }
